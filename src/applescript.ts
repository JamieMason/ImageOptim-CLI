...
export const isInstalled = (app: App): Promise<boolean> =>
  osascript(getScriptPath('is-installed'), app.bundleId).then(toBoolean);

export const quitApp = (app: App): Promise<void> =>
  osascript(getScriptPath('quit-app'), app.name).then(() => undefined);

export const runJpegMini = (app: App): Promise<void> =>
...
export const getJpegMini = async (): Promise<App | null> =>
